    ...
    <div className="min-h-screen bg-[#f7f3ec]/50">
      ...
      <div className="relative bg-gradient-to-br from-[#fefcf8] via-[#faf7f2] to-[#f5f0e8] border-b border-[#e8dfd0] overflow-hidden">
        ...
        <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 sm:py-12">
          <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-5">
            <div>
              <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-blue-50 text-blue-600 text-xs font-semibold rounded-full mb-3 tracking-wide">
                <FireOutlined />
                项目库
              </span>
            ...
            <div className="flex items-center gap-2">
              <div className="flex items-center gap-1 bg-[#f5f0e8] rounded-xl p-1.5">
                <button
                  onClick={() => setViewMode('grid')}
                  className={`p-2.5 rounded-lg transition-all duration-200 ${viewMode === 'grid' ? 'bg-white shadow-sm text-primary' : 'text-gray-400 hover:text-gray-600'}`}
                  title="网格视图"
                >
                  <AppstoreOutlined />
                </button>
                <button
                  ...
                  className={`p-2.5 rounded-lg transition-all duration-200 ${viewMode === 'list' ? 'bg-white shadow-sm text-primary' : 'text-gray-400 hover:text-gray-600'}`}
                  title="列表视图"
                >
                  <UnorderedListOutlined />
                </button>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* ═══════════════════════════════════════════════════════
              Filters — 搜索与筛选
            ═══════════════════════════════════════════════════════ */}
        <div className="dashboard-panel p-5 sm:p-6 mb-8">
          {/* 搜索栏 */}
          <form onSubmit={handleSearch} className="flex flex-col sm:flex-row gap-3 mb-6">
            <div className="flex-1 relative">
              ...
              <input
                ...
                className="w-full pl-11 pr-4 py-3.5 bg-[#faf7f2] border border-[#e8dfd0] rounded-xl focus:outline-none focus:ring-2 focus:ring-primary/15 focus:border-primary/30 focus:bg-white transition-all text-sm"
              />
            </div>
            <button
              type="submit"
              className="px-6 py-3.5 bg-primary text-white rounded-xl hover:bg-primary-light transition-all duration-300 font-semibold shadow-sm hover:shadow-md hover:-translate-y-0.5 text-sm flex items-center justify-center gap-2"
            >
              <SearchOutlined />搜索
            </button>
          </form>
              ...
              {STAGE_OPTIONS.map(option => (
                <button
                  ...
                  className={`px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${stageFilter === option.value
                      ...
                      : 'bg-[#f5f0e8] text-gray-600 hover:bg-[#efe8dc] hover:text-gray-800'
                    }`}
                >
                  {option.label}
                </button>
            ...
            <div className="flex flex-wrap gap-2">
              <button
                ...
                className={`px-3.5 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${industryFilter === ''
                    ? 'bg-accent-gradient text-primary shadow-md shadow-amber-500/15'
                    : 'bg-[#f5f0e8] text-gray-500 hover:bg-[#efe8dc] hover:text-gray-700'
                  }`}
              >
                全部领域
              </button>
          ...
          {hasActiveFilters && (
            <button
              ...
              className="mt-5 inline-flex items-center gap-1.5 text-xs font-medium text-gray-400 hover:text-red-500 transition-colors"
            >
        ...
        {loading ? (
            ...
            <div className={`grid gap-5 sm:gap-6 ${
              ...
              {projects.map((project, index) => (
                <Link
                  ...
                  className={`group bg-[#fefcf8]/90 backdrop-blur-sm rounded-2xl shadow-[0_2px_12px_rgba(10,42,92,0.03)] hover:shadow-[0_16px_40px_rgba(10,42,92,0.08)] transition-all duration-500 overflow-hidden border border-[#e8dfd0] hover:-translate-y-1.5 card-enter ${viewMode === 'list' ? 'flex flex-row' : ''}`}
                >
                  {/* 封面 */}
                  <div className={`relative flex items-center justify-center overflow-hidden bg-gradient-to-br from-[#f5f0e8] to-[#efe8dc] ${
                    viewMode === 'grid'
                      ? 'h-48'
                    ...
                    {project.cover_image ? (
                      ...
                      <div className="flex flex-col items-center gap-3">
                        <div className="w-16 h-16 rounded-2xl bg-white/60 flex items-center justify-center group-hover:scale-110 transition-transform duration-300">
                          <RocketOutlined className="text-3xl text-primary/20 group-hover:text-primary/35 transition-colors duration-300" />
                        </div>
                      </div>
                    )}
                    {/* 阶段角标 */}
                    <div className="absolute top-3 right-3">
                      <span className={`px-3 py-1.5 bg-gradient-to-r ${STAGE_GRADIENTS[project.stage] || 'from-gray-500 to-gray-600'} text-white text-xs font-semibold rounded-full shadow-lg`}>
                  ...
                  <div className={`${viewMode === 'grid' ? 'p-5 sm:p-6' : 'p-5 flex-1 flex flex-col justify-between'}`}>
                    <div>
                      <h3 className="text-lg font-bold text-primary group-hover:text-primary-light transition-colors duration-300 mb-2 line-clamp-1">
                        {project.title}
                      </h3>
                    ...
                    <div className="flex items-center justify-between text-xs text-gray-400 pt-4 border-t border-[#f0e8db]">
          ...
          <div className="text-center py-24">
            ...
            {hasActiveFilters && (
              <button
                onClick={() => { setStageFilter(''); setIndustryFilter(''); }}
                className="mt-4 inline-flex items-center gap-1.5 text-accent text-sm font-medium hover:underline"
              >
                <ClearOutlined /> 清空筛选
              </button>
            )}
        ...
        {hasMore && (
          <div className="text-center mt-12">
            <button
              ...
              className="inline-flex items-center gap-2 px-8 py-3.5 rounded-xl border-2 border-[#e8dfd0] text-primary font-semibold hover:border-primary/30 hover:bg-[#fefcf8] hover:shadow-md transition-all duration-300 group disabled:opacity-50 disabled:cursor-not-allowed"
            >
              {loadingMore ? (
                <>
