  ...
  return (
    <div className="p-6 lg:p-8">
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={(e) => { if (e.target === e.currentTarget) resetForm(); }}>
          <div className="bg-white rounded-2xl shadow-xl max-w-lg w-full p-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">资源描述</label>
                <textarea
                  required
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {resources.length === 0 ? (
          <div className="col-span-full bg-white rounded-xl shadow-custom border border-gray-100 p-12 text-center text-gray-400">
          ...
          resources.map((res: Resource) => (
            <div key={res.id} className="bg-white rounded-xl shadow-custom border border-gray-100 p-6 hover:shadow-custom-lg transition-all">
              ...
              <div className="border-t border-gray-100 pt-3 flex items-center justify-between text-xs">
                <span className="text-gray-400">联系人: {res.contact}</span>
                <span className="text-gray-400">{new Date(res.created_at).toLocaleDateString('zh-CN')}</span>
              </div>
            </div>
          ))
        )}
